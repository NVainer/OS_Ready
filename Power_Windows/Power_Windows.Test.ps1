...
function Wait-OfficeInstall {
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        Write-Log "Office apps: $($present.Count)/$($core.Count) [$($present -join ',')]; installer running: $([bool]$c2r)" DEBUG
...
Invoke-Step "17. Install + activate Office" { Install-Office } -Mutating
